...
import { BlogContext, FullDataContext } from '../App';
...
function NewsComponent({news}) {
    ...
    useEffect(() => {
        console.log("blog =", (blog?.category)?.charAt(0) )
    },[blog])
  ...
  return (
    ...
    <div className = "m-20">
    <div>  
    <h2 className = "flex justify-self-start ">{ (blog?.category)?.charAt(0).toUpperCase() + (blog?.category)?.slice(1)}  News</h2>
    </div>
    <div className = "flex mt-3 border-b-2 pb-6 ">
        ...
        <div className = "ml-5 flex-col ">
            <h3 className = "text-left">{(blog?.data?.[0])?.title}</h3>  
            <div className = "text-right mt-10">
            <h5 className = "text-gray-400">{(blog?.data?.[0])?.author+"at"+(blog?.data?.[0])?.time+" "+ "on" +(blog?.data?.[0])?.date}</h5>  
            </div>
            <div className = " mt-10 text-left">
                <h6>{(blog?.data?.[0])?.content}</h6>
            </div>
            <div>
